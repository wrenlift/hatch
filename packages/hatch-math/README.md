...
Operator overloads cover the obvious cases (`+`, `-`, `*`, unary `-`); every binary op returns a fresh instance. For hot loops where allocation is unwelcome, every type exposes in-place companions (`addInto(a, b)`, `mulInto(a, s)`) that write into `this`.
...
> **Note: `==` is exact**
> Equality compares bit-for-bit. After accumulated floating-point error, two "equal" values might disagree at the last few ulps. Use `Math.approxEq(a, b, eps)` or the `approxEq` method on each type when fuzzy comparison is the right fit.

## Compatibility

Wren 0.4 and WrenLift runtime 0.1 or newer. Pure-Wren, with no native dependencies. `@hatch:gpu` reuses these types directly via their `.data` accessor for batched buffer uploads, so manual flattening is unnecessary.
