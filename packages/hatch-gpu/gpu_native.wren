...
/// fragments + their own struct / binding / entry-point code; the
/// factory concatenates them in declaration order and hands the
/// result to `Device.createShaderModule`. This keeps per-pipeline
...
/// -- Material ----------------------------------------------------
///
/// Cook-Torrance / GGX PBR material in the glTF metallic-roughness
/// workflow. Each property pairs a *factor* (scalar / colour
/// multiplier) with an optional *texture* — when no texture is
/// bound the renderer substitutes a 1×1 default and the factor
/// becomes the final value. The texture's contribution is always
/// multiplied with the factor, matching the glTF 2.0 spec.
///
/// ## Example
///
/// ```wren
...
/// ```
///
/// Mutations are cheap. The renderer rebuilds the material's bind
/// group lazily when a property has changed since the last draw;
/// untouched materials reuse their cached bind group.
class Material {
  ...
  /// Internal — pack the material's plain-old-data into the 64-byte
  /// uniform block the WGSL shader expects. Texture handles aren't
  /// part of this — they bind separately. Appends 16 floats to
  /// `out` (4 vec4 rows).
  packUniform_(out) {
