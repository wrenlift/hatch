...
//
//   window_native.wren — winit-backed cdylib (macOS / Linux /
//                        Windows). `Window` class wraps a polled
//                        event loop, exposes a raw-window-handle
//                        Map @hatch:gpu's Surface accepts.
//
//   window_web.wren    — DOM-canvas backed wasm plugin (browser).
//                        `Window` static class creates a canvas
//                        under `#stage` (or document body) and
