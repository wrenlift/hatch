...
> v4 keys are uniformly distributed, which thrashes B-tree pages on insert. v7 keys carry a millisecond timestamp prefix, so new rows land near each other on disk. The collision space stays effectively infinite, and indexes stay hot.

## Compatibility

Wren 0.4 with WrenLift runtime 0.1 or newer. Native only; `#!wasm` builds need a separate WASM-compiled UUID bridge. Pair with `@hatch:sqlite` or `@hatch:json` when persisting or serialising.
