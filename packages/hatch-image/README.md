...
> RGBA8 means four bytes per pixel. If the source data is RGB, expand it to RGBA (alpha 255 for opaque) before calling `encode`. Passing a 3-channel buffer aborts with a length-mismatch error.

## Compatibility

Wren 0.4 and WrenLift runtime 0.1 or newer. Native only; `#!wasm` builds reach for browser-side decoders through `@hatch:web`. Pair with `@hatch:gpu` for texture upload, with `@hatch:assets` for hot-reloadable image content.
