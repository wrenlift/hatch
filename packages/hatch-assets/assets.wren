...
// | Backend               | Behaviour                                              |
// |-----------------------|--------------------------------------------------------|
// | `assets_native.wren`  | Filesystem-backed (`Fs.walk` + content hashes + `Hatch.watchFile` hot reload). Used on native; also driven by the spec runner. |
// | `assets_web.wren`     | Manifest-backed (fetch-driven, lazy per-asset reads, no filesystem watch). The page ships an `assets-manifest.json` next to the assets the runtime will reach for. |
//
