...
- ~~**PostFX inheritance blocker**~~ RESOLVED. The wlift class-field codegen fix for cross-module inheritance landed (panic on unresolved superclass at `src/mir/builder.rs:1786-1798`; hatch build threads dependency field layouts via `build_recursive` at `src/hatch.rs:1129-1140 / 1301-1302 / 1409-1410`). Procedural-world has PostFX re-enabled via `PostFxSetup` (Tonemap + Vignette) and builds + runs clean.
- ~~**GPU loop runaway memory**~~ RESOLVED 2026-05-28. Per-frame wrapper nulling in `RenderPass.end`, `SurfaceFrame.present`, and `Game.run`'s `encoder.destroy` keeps ecs-cubes bounded at 98–115 MB across 60s sampling. See `memory/project_gpu_loop_runaway_memory.md`. Revisit only on a fresh repro.
- **Bare var branch assign aliasing** — DOCUMENTED LIMITATION. Late-in-method locals can slot-alias each other under wlift codegen (specifically: `var t = <ternary>` or `var t; if (cond) t = X else t = Y` deep into a long method body). Reliable workaround: instance field or small helper method instead of a deep local — both bypass the slot-table path that triggers the alias. The deeper regalloc fix is tracked but not blocking (workaround is one-liner; bug requires very specific slot pressure conditions that minimal reproducers don't exercise). See `memory/feedback_bare_var_branch_assign.md`.
